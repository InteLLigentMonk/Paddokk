...
public class CarsController(ICarService carService) : ControllerBase
{
    ...
    public async Task<CarMakesResponse> GetCarMakes(CancellationToken cancellationToken) =>
        new() { Makes = [.. await _carService.GetCarMakesAsync(cancellationToken)] };

    [HttpGet("makes/{makeId}/models")]
    [EndpointSummary("Returns list of all car models for a specific make")]
    public async Task<CarModelsResponse> GetCarModels(int makeId, CancellationToken cancellationToken) =>
        new() { Models = [.. await _carService.GetCarModelsByMakeAsync(makeId, cancellationToken)] };

    [HttpGet("models/{modelId}/generations")]
    [EndpointSummary("Returns list of all car generations for a specific model")]
    public async Task<CarGenerationsResponse> GetCarGenerations(int modelId, CancellationToken cancellationToken) =>
        new() { Generations = [.. await _carService.GetCarGenerationsByModelAsync(modelId, cancellationToken)] };
